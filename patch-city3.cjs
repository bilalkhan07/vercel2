...
const startIdx = code.indexOf('function renderCityDynamicContent() {');
const endIdx = code.indexOf('function populateCityOfficeInfo() {');

if (startIdx !== -1 && endIdx !== -1) {
    const newRenderFn = `      function renderCityDynamicContent() {
        try {
          let services = [];
    ...
    code = code.substring(0, startIdx) + newRenderFn + code.substring(endIdx);
