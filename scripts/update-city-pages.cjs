...
${portfolioCardsHtml}
    ...
    <script>
        ...
        try {
          ...
          if (srvContainer && Array.isArray(services) && services.length > 0) {
            ...
            srvContainer.innerHTML = services.map(s => {
              ...
              const img = (s.image && !s.image.includes('.png')) ? s.image : 'https://images.unsplash.com/photo-1611162617213-7d7a39e9b1d7?w=700&auto=format&fit=crop&q=80';
              const targetSlug = slugMap[s.id] || slugMap[s.category] || 'graphic-designer';
              const linkUrl = targetSlug + '-in-' + currentCityKey + '.html';
              const iconSvg = getCityClientBadgeIcon(s);

              return '<div class="group rounded-2xl sm:rounded-3xl bg-white border border-slate-200 overflow-hidden hover:border-blue-300 hover:shadow-md transition-all flex flex-col justify-between shadow-xs">' +
                '<div>' +
                  '<div class="relative h-28 sm:h-48 bg-slate-100 overflow-hidden">' +
                    '<img src="' + img + '" alt="' + s.title + ' in ' + currentCityName + '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onerror="handleCityImgFallback(this)" />' +
                    '<div class="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>' +
                    '<div class="absolute top-2 left-2 sm:top-3 sm:left-3 w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-white/95 backdrop-blur-xs border border-white/60 flex items-center justify-center shadow-sm">' +
                      iconSvg +
                    '</div>' +
                  ...
                  '</div>' +
                  '<div class="p-3.5 sm:p-6 space-y-1.5 sm:space-y-2">' +
                    '<h3 class="text-sm sm:text-xl font-extrabold text-slate-950 leading-snug tracking-tight font-display">' + s.title + '</h3>' +
                    '<p class="text-xs sm:text-sm font-semibold text-slate-700 leading-snug line-clamp-2">' + (s.description || '') + '</p>' +
                  '</div>' +
                '</div>' +
                '<div class="p-3.5 sm:p-6 pt-0">' +
          ...
          if (portContainer && Array.isArray(portfolio) && portfolio.length > 0) {
            const displayPort = portfolio.slice(0, 4);
            portContainer.innerHTML = displayPort.map(p => {
              const pImg = p.image || 'https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=700&auto=format&fit=crop&q=80';
              return '<div class="group rounded-2xl sm:rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-xs hover:shadow-md transition-all flex flex-col justify-between">' +
                '<div>' +
                  '<div class="relative h-28 sm:h-48 bg-slate-100 overflow-hidden">' +
                    '<img src="' + pImg + '" alt="' + p.title + '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onerror="handlePortImgFallback(this)" />' +
                    '<span class="absolute bottom-2 right-2 sm:bottom-3 sm:right-3 bg-white/95 text-emerald-800 text-[9px] sm:text-xs font-extrabold px-2 py-0.5 rounded-full shadow-xs">' + (p.deliveryTime || p.delivery || '⚡ 35m') + '</span>' +
                  '</div>' +
                  '<div class="p-3.5 sm:p-5 space-y-1">' +
                    '<span class="text-[10px] uppercase font-bold text-blue-600 tracking-wider block">' + (p.category || 'Graphic Design') + '</span>' +
                    '<h3 class="font-extrabold text-slate-950 text-sm sm:text-base leading-snug tracking-tight font-display">' + p.title + '</h3>' +
                    '<p class="text-xs font-semibold text-slate-600 line-clamp-2 leading-snug">' + (p.description || p.desc || '') + '</p>' +
                  '</div>' +
                '</div>' +
                '<div class="p-3.5 sm:p-5 pt-0">' +
                  '<a href="request.html?city=' + encodeURIComponent(currentCityName) + '&service=' + encodeURIComponent(p.category || 'graphic-design') + '" class="w-full text-center py-2 sm:py-2.5 rounded-xl bg-slate-100 hover:bg-blue-600 hover:text-white text-slate-900 text-xs font-extrabold transition-all block">' +
          ...
          console.warn('Dynamic city content render note:', e);
        }
      }

      // Populate Admin-Configured City Office Address & Details
          ...
          return '<div class="bg-white rounded-3xl border border-slate-200 shadow-xs p-6 hover:shadow-md hover:border-blue-300 transition-all duration-300 flex flex-col justify-between">' +
            ...
            '<div class="pt-4 border-t border-slate-100 flex items-center justify-between gap-3">' +
              '<div class="flex items-center gap-3 overflow-hidden">' +
                '<img src="' + avatar + '" alt="' + name + '" class="w-11 h-11 rounded-full object-cover border-2 border-amber-400 shrink-0 shadow-2xs" onerror="this.onerror=null; this.src=\'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80\';" />' +
                '<div class="truncate">' +
      ...
      // Real-time backend sync from Supabase/Firebase if active
      async function syncCityData() {
        ...
        if (db && typeof db.subscribeReviews === 'function') {
          try {
